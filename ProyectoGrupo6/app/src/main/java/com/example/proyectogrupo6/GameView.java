...
public class GameView extends View {
    ...
    public GameView(Context context, @Nullable AttributeSet attrs) {
        ...
        if(sp!=null){
            bestscore = sp.getInt("MEJOR PUNTUACIÓN", 0);
        }
        score = 0;
        start = false;
        initBird();
    ...
    private void initPipe() {
        sumpipe = 5;
        distances = 500 * Constants.SCREEN_HEIGHT/1920;
        arrPipes = new ArrayList<>();
        for (int i = 0; i < sumpipe; i++){
            if(i < sumpipe/2){
                this.arrPipes.add(new Pipe(Constants.SCREEN_WIDTH+i*((Constants.SCREEN_WIDTH+200*Constants.SCREEN_WIDTH/1880)/(sumpipe/2)),
                        0, 250*Constants.SCREEN_WIDTH/1080, Constants.SCREEN_HEIGHT/2));
                ...
                this.arrPipes.add(new Pipe(this.arrPipes.get(i-sumpipe/2).getX(), this.arrPipes.get(i-sumpipe/2).getY()
                        +this.arrPipes.get(i-sumpipe/2).getHeight() + this.distances, 250*Constants.SCREEN_WIDTH/1080, Constants.SCREEN_HEIGHT/2));
    ...
    private void initBird() {
        bird = new Bird();
        bird.setWidth(165*Constants.SCREEN_WIDTH/1080);
        bird.setHeight(145*Constants.SCREEN_HEIGHT/1920);
        bird.setX(100*Constants.SCREEN_WIDTH/1080);
        bird.setY(Constants.SCREEN_WIDTH/2-bird.getHeight()/2);
        ...
        if(start){
            ...
            for (int i = 0; i < sumpipe; i++){
                if(bird.getRect().intersect(arrPipes.get(i).getRect())||bird.getY()-bird.getHeight()<0||bird.getY()>Constants.SCREEN_HEIGHT){
                    Pipe.speed = 0;
                    MainFlappy.txt_score_over.setText(MainFlappy.txt_score.getText());
                    MainFlappy.txt_best_score.setText("MEJOR PUNTUACIÓN: " + bestscore);
                    MainFlappy.txt_score.setVisibility(INVISIBLE);
                    MainFlappy.rl_game_over.setVisibility(VISIBLE);
                }
                if(this.bird.getX()+this.bird.getWidth()>arrPipes.get(i).getX()+arrPipes.get(i).getWidth()/2
                    ...
                    if(score>bestscore){
                        ...
                        editor.putInt("MEJOR PUNTUACIÓN", bestscore);
                        editor.apply();
                    }
                    MainFlappy.txt_score.setText(""+score);
                ...
                if(this.arrPipes.get(i).getX() <- arrPipes.get(i).getWidth()){
                    ...
                    if (i < sumpipe/2){
                        ...
                        arrPipes.get(i).setY(this.arrPipes.get(i-sumpipe/2).getY()
                                +this.arrPipes.get(i-sumpipe/2).getHeight() + this.distances);
                    }
                }
    ...
    public void reset(){
        MainFlappy.txt_score.setText("0");
        score = 0;
        initPipe();
        initBird();
    }
